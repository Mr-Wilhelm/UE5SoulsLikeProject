...
void UTraceComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (const FTraceSockets Socket : Sockets) //const means that we dont plan to edit the variable in the loop
	{
		FVector socketStartPos{ skeletalComp->GetSocketLocation(Socket.socketStart) };
		FVector socketEndPos{ skeletalComp->GetSocketLocation(Socket.socketEnd) };
		FQuat socketRotationPos{ skeletalComp->GetSocketQuaternion(Socket.socketRotation) };
		...
		bool hasFoundTargets{ GetWorld()->SweepMultiByChannel(outResults,
			...
			Box,
			ignoreParams) };	//sweep trace using the box shape

		for (FHitResult Hit : outResults)
		{
			AllResults.Add(Hit);	//add the hit result to the array of all results
		}
	...
	if (AllResults.Num() == 0) { return; }	//if nothing is hit, return early
	...
	for (const FHitResult& hit: AllResults)	//foreach loop, adding & ensures that we get a reference to the variable, which is faster than making a copy
	{
